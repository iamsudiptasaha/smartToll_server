...
@author : Sudipta Saha
@since : 23rd Jan, 2018
This page returns information about a tour based on tourid.
*/
...
if(isset($_POST['username']) && isset($_POST['tourid'])){
    if(!empty($_POST['username']) && !empty($_POST['tourid'])){
            ...
            require 'DatabaseConnect.smartToll.php';
              if(makeConnection()){
                 //Get data from database
                  $query="SELECT cusrecord.username, $db_tablename.* FROM $db_tablename INNER JOIN `cusrecord` ON cusrecord.id=$db_tablename.userid WHERE username=\"".mysqli_real_escape_string($db_con,$username)."\" AND tourid=\"".mysqli_real_escape_string($db_con,$tourid)."\"";
...
if($successvalue=='false'){
  $result= array('success' => $successvalue, 'errorCode' => (string)$errorCode);
    ...
    $result= array('success' => $successvalue, 'errorCode' => (string)$errorCode, 'tourid'=>$tourid, 'userid'=>$userid, 'username'=>$username,'station_from'=>$station_from,'entry_time'=>$entry_time,'station_to'=>$station_to,'exit_time'=>$exit_time,'status'=>$status, 'tourcost'=>$tourcost);
